...
# C# 插件监听的本地端口
REVIT_LOCAL_URL = "http://localhost:8081/mcp/"

def send_to_revit(command_name: str, args: dict) -> str:
    """向本地 Revit C# 插件发送指令"""
    data = json.dumps({
        "command": command_name,
        "args": args
    }).encode('utf-8')
    req = urllib.request.Request(REVIT_LOCAL_URL, data=data, headers={'Content-Type': 'application/json'})
    try:
...
async def handle_call_tool(name: str, arguments: dict) -> list[TextContent]:
    if name == "run_revit_command":
        res = send_to_revit(arguments["commandName"], arguments["args"])
        return [TextContent(type="text", text=res)]
    elif name == "run_pyrevit_script":
        res = send_to_revit("eval_python", {"script": arguments["scriptContent"]})
        return [TextContent(type="text", text=res)]
    else:
